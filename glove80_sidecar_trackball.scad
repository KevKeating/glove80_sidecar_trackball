...
show_mouse_buttons_shell = true;
show_mouse_buttons_back_wall = true;
show_mouse_buttons_upper_pcb_holder = true;
show_rear_wall = true;
show_side_wall = true;
show_pcb_clips = true;
show_trackball_holder = true;
show_trackball_holder_pcb_mount = true;
show_battery_compartment_cover = true;
...
show_light_cover = true;
show_mouse_wheel_2p_inner = false;
show_mouse_wheel_2p_outer = false;
show_mouse_wheel_1p = true;
...
static_bearing_mount_additional_height = 0.25;
...
/* [Mouse button shape] */
mouse_buttons_straight_width = 16;
mouse_buttons_angled_width = 9.5;
mouse_button_curve_rounding = 20;
mouse_button_top_and_bottom_rounding = 1.5;
mouse_button_thickness = 2.5;
mouse_button_shell_top_thickness = 1.5;
mouse_button_shell_bottom_thickness = 2;
mouse_buttons_additional_height = 8.75;
height_past_left_mouse_button = 9;
thin_mouse_buttons_palm_wall = 1.35;
thin_mouse_buttons_thumb_tip_wall = 0.75;
...
// Increase (or decrease) the values in this section slightly if the threading is too tight (or loose) for the screw
assembly_screw_slop = 0.1025;
// the slop for assembly screws that are printed horizontally (i.e. the screw goes in parallel to the build plate)
horizontal_assembly_screw_slop = 0.0385;
// the slop for the threaded rod attaching to the foot
foot_screw_slop = 0.115;
// The screws for mounting the PCB with the button switches
mouse_button_screw_slop = 0.0925;
...
// the slop for the printed connection light cover
...
mouse_buttons_pcb_y_offset = mouse_buttons_additional_height - wall_thickness - mouse_button_shell_top_thickness;
...
mouse_button_palm_end_droop = 10;
...
mouse_wheel_rounding = 1;
mouse_wheel_elevation = 9;
mouse_wheel_opening_radius = 10.5;
...
mouse_button_width_right = 17;
mouse_button_rounding = 4;
width_between_mouse_buttons = 2.5;
mini_mouse_button_length = 35;
...
mouse_buttons_pcb_wall_cable_passthrough_height = 7.5;
mouse_buttons_pcb_wall_cable_passthrough_width = 15;

mouse_wheel_inner_width = 12.5;
mouse_wheel_outer_diameter = 19.5;
mouse_wheel_thickness = 9.25;
mouse_wheel_rib_size = 2;
mouse_wheel_hex_diam = 1.675; // inner diameter
...
dongle_holder_width = 12.2;
dongle_holder_height = 4.7;
...
battery_compartment_cover_floor_depth = 11;
battery_compartment_cover_floor_catch_thickness = 2.5;
battery_compartment_cover_floor_catch_depth = 4;
...
trackball_holder_pcb_opening_length = 10;
trackball_holder_pcb_opening_width = 8.5;
trackball_holder_pcb_opening_rounding = 1.5;
trackball_holder_pcb_mounting_dist = 6.25;
sensor_pcb_mounting_height = 3;
sensor_mounting_height = 6;
trackball_holder_split_clearance = 0.02;
...
connection_button_peg_clearance = 0.5;
    ...
    zrot(
        ...
        [for (i = [0:1 / min($fn, 100):1])
            right(
                ...
                zrot(
                    90 * i,
                    [mouse_buttons_shell_path(mouse_buttons_palm_end_scaling(i), "bottom_x_only") - mouse_buttons_back_thickness,
                    0]
                ))]));
    ...
    diff() {
        ...
        intersection() {
            ...
            translate(trackball_center) {
                ...
                // hollow out the sphere to make room for the trackball, but leave a little extra room for the static bearing mounts
                    ...
                    zrot_copies(n=3, sa=-(180 - sidecar_angle))
                        yrot($idx ? -trackball_bearing_vertical_angle : -trackball_back_bearing_vertical_angle) {
                            ...
                            cyl(d=static_bearing_diameter, h=trackball_diameter / 2 + static_bearing_diameter - static_bearing_exposed_height,
                                ...
                                anchor=TOP);
                            // add a little extra height around the static bearings so that they don't fall out too easily
                            tag("inner_remove")
                                down(trackball_diameter / 2 + static_bearing_exposed_height + EXTRA)
                                tube(id=static_bearing_diameter,
                                     od1=static_bearing_diameter + 1 + static_bearing_mount_additional_height,
                                     od2=static_bearing_diameter + 1,
                                     h=static_bearing_mount_additional_height + EXTRA,
                                     anchor=BOTTOM);
                        }
                }
                tag("remove") {
...
module bottom_shell() {
    ...
    bottom_floor_path = flatten([
        bottom_floor_tat,
        [
         [mouse_buttons_shell_rear_corner.x + wall_thickness, mouse_buttons_shell_rear_corner.y + mouse_buttons_back_thickness * cos(thumb_cluster_wall_angle)],
            ...
            cuboid([assembly_screw_receiver_diameter + assembly_screw_receiver_diameter * top_x_offset / sidecar_angled_portion_height,
                ...
                position(LEFT + BACK + TOP)
                    translate([assembly_screw_receiver_diameter / 2,
                        ...
                        assembly_screw_hole_threaded(BACK, slop=horizontal_assembly_screw_slop);
            }
    ...
    brace_length = glove80_thumb_cluster_length - palm_end_length - 2.5;
        ...
        cuboid([thumb_cluster_lower_brace_thickness, brace_length, brace_height + EXTRA],
            ...
            position(LEFT + TOP + FRONT)
                ...
                cuboid([assembly_screw_threading_total_length, assembly_screw_receiver_diameter, assembly_screw_receiver_diameter],
                    ...
                    position(LEFT)
                        assembly_screw_hole_threaded(LEFT, slop=horizontal_assembly_screw_slop);
                    position(BOTTOM)
                ...
                translate([0, assembly_screw_receiver_diameter / 2, assembly_screw_receiver_diameter / 2]) {
                    ...
                    tag_diff("keep", "inner_remove", "inner_keep")
                        cuboid([assembly_screw_threading_total_length, assembly_screw_receiver_diameter, assembly_screw_receiver_diameter],
                            ...
                            anchor=LEFT)
                            position(LEFT)
                                assembly_screw_hole_threaded(LEFT, tag="inner_remove", slop=horizontal_assembly_screw_slop);
                }
        }
    ...
    translate(point3d(assembly_screw_bottom_shell_mouse_buttons_back_wall_loc, -case_height + bottom_floor_thickness))
        ...
        cuboid([assembly_screw_receiver_diameter, assembly_screw_threading_total_length, assembly_screw_receiver_diameter],
               ...
               anchor=FRONT + BOTTOM)
            position(FRONT)
                assembly_screw_hole_threaded(FRONT, slop=horizontal_assembly_screw_slop);
}


module assembly_screw_hole_threaded(orient=DOWN, tag="remove", length=assembly_screw_threading_length, slop=assembly_screw_slop) {
    tag(tag)
        screw_hole(assembly_screw_type, thread=true, length=length, $slop=slop, orient=orient, anchor=TOP);
}
...
module exposed_assembly_screw_hole_unthreaded(length=mouse_buttons_back_thickness, orient=DOWN, anchor=TOP) {
    tag("remove")
...
module thumb_cluster_wall_palm_end(extra_width=0) {
    mouse_buttons_path = mouse_buttons_shell_path(mouse_buttons_palm_end_scaling(1), "inner_only", lowered_top=mouse_button_palm_end_droop, thin_wall=thin_mouse_buttons_palm_wall);
    translate(point3d(mouse_buttons_shell_wall_start + straight_shell_length * thumb_cluster_unit_vector, mouse_buttons_additional_height))
        ...
        left(mouse_buttons_back_thickness)
        linear_extrude(mouse_buttons_back_thickness + extra_width)
        ...
        diff()
            cuboid([assembly_screw_threading_total_length + top_shell_inner_assembly_screw_receiver_chamfer_x, assembly_screw_receiver_diameter, assembly_screw_receiver_diameter],
                ...
                edge_profile(RIGHT + BOTTOM)
                    ...
                    mask2d_chamfer(x=top_shell_inner_assembly_screw_receiver_chamfer_x, y=assembly_screw_receiver_diameter);
                position(RIGHT)
                    assembly_screw_hole_threaded(RIGHT, length=assembly_screw_threading_length + top_shell_inner_assembly_screw_receiver_chamfer_x, slop=horizontal_assembly_screw_slop);
            }
...
module top_shell() {
    ...
    diff() {
        ...
        intersection() {
            ...
            translate([trackball_center.x, trackball_center.y, -top_shell_thickness]) {
                tube(od=trackball_wall_outer_diameter + 2 * (top_shell_rim_radial_clearance + top_shell_rim_thickness),
                     wall=top_shell_rim_thickness,
                     h=top_shell_rim_height,
                     anchor=TOP);
...
// !union()
//     stroke(mouse_buttons_shell_path(), width=1, closed=true);

// path_type can be "shell", "inner_only", "bottom_x_only", "solid"
function mouse_buttons_shell_path(scaling=[1, 1], path_type="shell", lowered_top=0, thin_wall=0) =
    let (
        ...
        angled_height = mouse_button_width_right + height_past_left_mouse_button,
        outer_top = -(1 - scaling[0]) * lowered_top,
        thinned_mouse_button_thickness = mouse_button_thickness - thin_wall,
        
        inner_floor_pos_x = -straight_width - angled_width,
        straight_path = [
            [0, 0],
            [-straight_width, outer_top],
            // in between the upper and lower buttons
            [-straight_width, mouse_wheel_center_y],
            [-straight_width - angled_width, mouse_wheel_center_y - mouse_button_width_right - height_past_left_mouse_button],
            [0, mouse_wheel_center_y - mouse_button_width_right - height_past_left_mouse_button],
            // start of the interior of the shell
            [0, mouse_wheel_center_y - mouse_button_width_right - height_past_left_mouse_button + mouse_button_shell_bottom_thickness],
            [-straight_width - angled_width + thinned_mouse_button_thickness + thinned_mouse_button_thickness * angled_width / angled_height, mouse_wheel_center_y - mouse_button_width_right - height_past_left_mouse_button + mouse_button_shell_bottom_thickness],
            [-straight_width + thinned_mouse_button_thickness, mouse_wheel_center_y - thinned_mouse_button_thickness * angled_width / angled_height],
            [-straight_width + thinned_mouse_button_thickness, outer_top - mouse_button_shell_top_thickness - thinned_mouse_button_thickness * -outer_top / straight_width],
            [0, -mouse_button_shell_top_thickness],
        ],
        radii = [
            0,
...
function mouse_buttons_thumb_tip_end_scaling(i) = [
    1 + 1 * sin(90 * i^3.5) - (i > 0.9 ? (10 * (i - 0.9))^2 * 0.5 : 0),
    0.4 + 0.6 * (1 - 0.50 * i)^(5) - (i > 0.9 ? (10 * (i - 0.9))^2 * 0.41874 : 0)
];
            ...
            yrot(
                ...
                path3d(
                    mouse_buttons_shell_path(
                        ...
                        path_type=path_type,
                        thin_wall=thin_mouse_buttons_thumb_tip_wall)))];
...
module mouse_buttons_shell() {
    ...
    diff() {
        translate(point3d(mouse_buttons_shell_wall_start, mouse_buttons_additional_height)) {
            ...
            rotate([mouse_buttons_pcb_x_rotate, 180, thumb_cluster_wall_angle])
                translate([mouse_buttons_and_wheel_x_offset - 0.65 - new_x_offset,
                           -wall_thickness - mouse_button_shell_top_thickness + mouse_wheel_pcb_y + new_y_offset,
                           0]) {
                    up(mouse_buttons_pcb_z_offset + mouse_buttons_pcb_thickness + mouse_wheel_elevation)
                        back(0.5)
                        tag("remove")
                        // cutout for the mouse wheel
                        cuboid([mouse_wheel_opening_width, 2 * mouse_wheel_opening_radius, 75], rounding=mouse_wheel_rounding, edges="Z");
                    up(7)
                        // the up(7) ensures that we're clear of the back of the thumb tip end
                        mouse_button_cutouts();
                }
            intersection() {
                union() {
                    rotate([mouse_buttons_pcb_x_rotate, 180, thumb_cluster_wall_angle])
                        translate([mouse_buttons_and_wheel_x_offset - mouse_buttons_pcb_x_offset - new_x_offset,
                                    -wall_thickness - mouse_button_shell_top_thickness + new_y_offset,
        ...
        translate(point3d(mouse_buttons_shell_wall_start,
            ...
            zrot(thumb_cluster_wall_angle) {
                ...
                translate([assembly_screw_receiver_diameter / 2 - mouse_buttons_back_thickness * cos(thumb_cluster_wall_angle),
                           ...
                           -mouse_button_shell_bottom_thickness])
                    exposed_assembly_screw_hole_unthreaded(mouse_button_shell_bottom_thickness);
            }
        tag_intersect("keep", "intersect", "inner_keep") {
                ...
                zrot(thumb_cluster_wall_angle)
                    tag_diff("intersect", "inner_remove", "inner_inner_keep")  {
                        right(straight_shell_length - assembly_screw_mouse_buttons_top_front_x_offset)
                            ...
                            cuboid([2 * assembly_screw_receiver_diameter, assembly_screw_threading_total_length - 1, assembly_screw_receiver_diameter + 3],
                                    rounding=assembly_screw_receiver_rounding,
                                ...
                                position(LEFT + FRONT + TOP)
                                    cuboid([2 * assembly_screw_receiver_diameter - 6, 7, 7],
                                           chamfer=7,
                                           edges=FRONT + BOTTOM,
                                           anchor=LEFT + BACK + TOP);
                            }
            ...
            translate(point3d(mouse_buttons_shell_wall_start + straight_shell_length * thumb_cluster_unit_vector, mouse_buttons_additional_height))
                rotate([90, 0, thumb_cluster_wall_angle + 90]) {
                    ...
                    cuboid([20, 10, 10], anchor=RIGHT + BACK + TOP);
                }
        }
        // the upper assembly screw receiver on the main button side
        translate(point3d(mouse_buttons_shell_wall_start,
                          mouse_buttons_additional_height - mouse_button_shell_top_thickness))
            zrot(thumb_cluster_wall_angle)
                left(mouse_buttons_back_thickness * cos(thumb_cluster_wall_angle))
                tag_diff("keep", "inner_remove", "inner_keep")
                cuboid([assembly_screw_receiver_diameter, assembly_screw_threading_total_length, assembly_screw_receiver_diameter],
                    rounding=assembly_screw_receiver_rounding,
                    edges=RIGHT + BOTTOM,
                    anchor=LEFT + BACK + TOP) {
                        position(BACK)
                            assembly_screw_hole_threaded(BACK, tag="inner_remove", slop=shell_assembly_screw_slop);
                        position(FRONT + TOP)
                            cuboid([assembly_screw_receiver_diameter, 2, 2],
                                   chamfer=2,
                                   edges=FRONT + BOTTOM,
                                   anchor=BACK + TOP);
                        position(LEFT + TOP)
        ...
        wall_above_side_wall_length = mouse_buttons_thumb_tip_end_scaling(1)[0] * mouse_buttons_straight_width - mouse_button_thickness + thin_mouse_buttons_thumb_tip_wall + 0.5;
        tag_diff("keep", "inner_remove", "inner_keep") {
        ...
        // round the top of the wall above the side wall, including the extrusion
        translate([mouse_buttons_shell_wall_start.x, mouse_buttons_shell_wall_start.y + 1.5, mouse_buttons_additional_height])
            cuboid([mouse_button_thickness - thin_mouse_buttons_thumb_tip_wall,
                    wall_above_side_wall_length - 0.5,
                    mouse_button_shell_top_thickness],
                anchor=RIGHT + FRONT + TOP)
            edge_profile(RIGHT + TOP)
                tag("remove")
                mask2d_chamfer(x=sidecar_outer_edge_chamfer, y=sidecar_outer_edge_chamfer);
        // solidify the very end of the wall
        intersection() {
            mouse_buttons_thumb_tip_end_polyhedron("solid");
            translate([mouse_buttons_shell_wall_start.x, mouse_buttons_shell_wall_start.y + 15, mouse_buttons_additional_height - mouse_button_shell_top_thickness])
                zrot(-45)
                cuboid([25, 25, case_height], anchor=RIGHT + FRONT + TOP);
        }
            ...
            for (i = [0:1 / min($fn, 100):1]) 
                right(
                    ...
                    yrot(
                        ...
                        path3d(left(
                            mouse_buttons_back_thickness,
                            mouse_buttons_shell_path(mouse_buttons_palm_end_scaling(i), lowered_top=mouse_button_palm_end_droop, thin_wall=thin_mouse_buttons_palm_wall)))))];
            ...
            for (i = [0:1 / min($fn, 100):1]) 
                right(
                    ...
                    yrot(
                        ...
                        path3d(left(
                            mouse_buttons_back_thickness,
                            mouse_buttons_shell_path(mouse_buttons_palm_end_scaling(i), lowered_top=mouse_button_palm_end_droop, path_type="solid", thin_wall=thin_mouse_buttons_palm_wall)))))];
        ...
        // fill in the gap above the mouse button back wall
        top_start_point = mouse_buttons_shell_wall_start - 2.85 * thumb_cluster_unit_vector;
        translate(point3d(top_start_point, mouse_buttons_additional_height))
            rotate(thumb_cluster_wall_angle + 90) {
                cuboid([mouse_buttons_back_thickness,
                        ...
                        mouse_button_shell_top_thickness],
                       rounding=1.5,//mouse_button_thickness,
                       edges=LEFT + FRONT,
...
module mouse_button_cutouts() {
    ...
    extra_left_button_width = mouse_wheel_switch_width / 2 + mouse_wheel_mount_width / 2 + 2 * mouse_wheel_mount_cover_width + 0.1;
    tag_diff("remove", "inner_remove", "inner_keep") {
        // left button
        left_button_width = mouse_button_width_left - mouse_wheel_switch_width / 2 - mouse_wheel_mount_cover_width - mouse_wheel_mount_cover_clearance;
        translate([-mini_buttons_x_offset - mouse_button_cutout_width / 2 + width_between_mouse_buttons,
                   ...
                   -15]) {
            mouse_button([mouse_button_length + 30,
                          mouse_button_width_left - mouse_wheel_switch_width / 2 - mouse_wheel_mount_cover_width - mouse_wheel_mount_cover_clearance,
                          cutout_height], 
            ...
            tag("inner_remove")
                ...
                cuboid([20, extra_left_button_width, cutout_height],
                    ...
                    edges=BACK + LEFT,
                    anchor=FRONT + LEFT);
            // extend the button to include the top edge of the shell
            translate([0, -left_button_width, 19])
                cuboid([70, 20, 5],
                       rounding=1.5,
                       edges = LEFT + BOTTOM,
                       anchor=BACK + LEFT + BOTTOM);
            translate([mouse_button_cutout_width, -left_button_width, 19 + mouse_button_cutout_width])
                tag_diff("inner_remove", "inner_inner_remove", "inner_inner_keep")
                cuboid([70, 20, 5],
                       rounding=mouse_button_rounding - mouse_button_cutout_width,
                       edges = LEFT + BOTTOM,
                       anchor=BACK + LEFT + BOTTOM)
                    edge_profile(LEFT + BOTTOM)
                        tag("inner_inner_remove")
                        mask2d_chamfer(y=1.5 * mouse_button_cutout_width, x=62.5);
        }
        // right button
        translate([mouse_wheel_opening_width / 2 - mouse_button_cutout_width,
                  -(mouse_wheel_switch_width / 2 + mouse_wheel_mount_cover_width + mouse_wheel_mount_cover_clearance),
                  -15]) {
            length_adjustment = mini_buttons_x_offset + mouse_wheel_opening_width / 2 - mouse_button_cutout_width / 2 - width_between_mouse_buttons - 10;
            right_button_width = mouse_button_width_right + mouse_wheel_switch_width / 2 + mouse_wheel_mount_cover_width + mouse_wheel_mount_cover_clearance - mouse_button_cutout_width / 2;
            mouse_button([mouse_button_length - length_adjustment,
                          right_button_width,
                          cutout_height],
                         rounding=mouse_button_rounding,
                         edges=[],
                         anchor=FRONT + LEFT,
                         inner_anchor=RIGHT);
            back(right_button_width + mouse_button_cutout_width)
                cuboid([length_adjustment + 10, 10, cutout_height],
                       rounding=mouse_button_rounding,
                       edges=[LEFT + FRONT, LEFT + BACK],
                       anchor=BACK + RIGHT)
                    position(LEFT)
                        tag("inner_remove")
                        right(mouse_button_cutout_width)
                        cuboid([length_adjustment + 10, 10 - 2 * mouse_button_cutout_width, cutout_height],
                               rounding=mouse_button_rounding - mouse_button_cutout_width,
                               edges=[LEFT + FRONT, LEFT + BACK],
                               anchor=LEFT);
        }
        ...
        // right mini button
        translate([-mini_buttons_x_offset + mouse_button_cutout_width / 2, -mouse_button_cutout_width / 2])
            mouse_button([mini_mouse_button_length, mouse_button_width_right - 1, cutout_height],
                            rounding=mouse_button_rounding,
                            edges=BACK + RIGHT,
                            anchor=FRONT + RIGHT,
                            inner_anchor=LEFT);
    ...
    tag("remove") {
        ...
        back(1)
            cuboid([mouse_wheel_switch_length_from_center + mouse_wheel_mount_cover_width + mouse_wheel_mount_cover_clearance,
                    mouse_wheel_switch_width + 2 * (mouse_wheel_mount_cover_width + mouse_wheel_mount_cover_clearance) + 1,
                    cutout_height],
...
module mouse_buttons_back_wall_undecorated() {
    ...
    translate(point3d(bottom_shell_wall_end_plus_wall_thickness[1], mouse_buttons_additional_height - mouse_button_shell_top_thickness))
        rotate(thumb_cluster_wall_angle + 90) 
            translate([wall_thickness - mouse_buttons_back_thickness, mouse_buttons_additional_length + wall_thickness * sin(thumb_cluster_wall_angle), 0])
                ...
                cuboid([mouse_buttons_back_thickness,
                        wall_length - 0.01,
                        case_height - mouse_button_shell_top_thickness + mouse_buttons_additional_height],
                       anchor=TOP + LEFT + BACK)
                    children();
}
...
module mouse_buttons_back_wall() {
    diff() {
        mouse_buttons_back_wall_undecorated() {
            ...
            position(LEFT + BACK + TOP)
                up(mouse_wheel_pcb_y - mouse_buttons_pcb_y_offset - mouse_button_width_right - height_past_left_mouse_button + mouse_button_shell_top_thickness + mouse_button_shell_bottom_thickness)
                cuboid([assembly_screw_receiver_diameter + assembly_screw_mouse_buttons_bottom_rear_y_offset,
                    ...
                    position(LEFT + BOTTOM)
                        ...
                        assembly_screw_hole_threaded(slop=horizontal_assembly_screw_slop);
                    
            tag("remove") {
                edge_profile(RIGHT + BOTTOM)
                    mask2d_chamfer(x=0.4, y=0.4);
                ...
                position(LEFT + BACK + TOP)
                    translate([0, -assembly_screw_receiver_diameter / 2, -mouse_buttons_additional_height - assembly_screw_receiver_diameter / 2 - 1])
                        exposed_assembly_screw_hole_unthreaded(orient=LEFT);
                // the pass through for the cable
                position(BACK + TOP)
                    translate([0,
                               -assembly_screw_receiver_diameter,
                               mouse_wheel_pcb_y - mouse_buttons_pcb_y_offset - mouse_button_width_right - height_past_left_mouse_button + mouse_button_shell_top_thickness + mouse_button_shell_bottom_thickness])
                        cuboid([mouse_buttons_back_thickness + 2 * EXTRA,
                                mouse_buttons_pcb_wall_cable_passthrough_width,
                                mouse_buttons_pcb_wall_cable_passthrough_height],
                               rounding=2,
                               edges="X",
                               anchor=BACK + BOTTOM);
                ...
                position(RIGHT + FRONT + TOP)
                    translate([0,
                              -assembly_screw_receiver_diameter / 2 + assembly_screw_mouse_buttons_top_front_x_offset + mouse_button_shell_top_thickness,
                              mouse_wheel_pcb_y - mouse_buttons_pcb_y_offset + assembly_screw_mouse_buttons_top_front_z_offset + mouse_button_shell_top_thickness + assembly_screw_receiver_diameter / 2])
                        exposed_assembly_screw_hole_unthreaded(orient=RIGHT);
                // the screw hole for the lower front assembly screw that connects to the mouse button shell
                position(RIGHT + FRONT + TOP)
                    translate([0,
                              -assembly_screw_receiver_diameter / 2 + mouse_button_shell_top_thickness - assembly_screw_mouse_buttons_bottom_front_x_offset,
                              mouse_wheel_pcb_y - mouse_buttons_pcb_y_offset - mouse_button_width_right - height_past_left_mouse_button + mouse_button_shell_bottom_thickness + mouse_button_shell_top_thickness + assembly_screw_receiver_diameter / 2])
                        exposed_assembly_screw_hole_unthreaded(orient=RIGHT);
    ...
    translate(point3d(mouse_buttons_shell_wall_start))
        intersection() {
            rotate([mouse_buttons_pcb_x_rotate, 180, thumb_cluster_wall_angle])
                translate([mouse_buttons_and_wheel_x_offset - mouse_buttons_pcb_x_offset - new_x_offset,
                          mouse_buttons_pcb_y_offset + new_y_offset,
...
module mouse_buttons_pcb_mount(height) {
    ...
    translate([mouse_wheel_pcb_x - 6.5, mouse_wheel_pcb_y, -height])
        diff()
...
mouse_button_switch_pad_bar_width = 1;
module mouse_buttons_switch_pads(height) {
    translate([1.5, -7.25, -mouse_button_mini_pad_reduced_elevation + mouse_button_switch_pad_bar_height])
        // right mini mouse button pad
        cuboid([mouse_button_mini_pad_width + 0.25, mouse_button_mini_pad_width + 14, height],
               rounding=3,
               edges=RIGHT + BACK,
               anchor=LEFT + BOTTOM)
            position(BOTTOM)
                cuboid([mouse_button_switch_pad_bar_width,
                        mouse_button_mini_pad_width + 13,
                        mouse_button_switch_pad_bar_height],
                       anchor=TOP);
    translate([1.5, -14.25 + mouse_button_mini_pad_width / 2, -mouse_button_mini_pad_reduced_elevation + mouse_button_switch_pad_bar_height])
        // left mini mouse button pad
        cuboid([mouse_button_mini_pad_width + 0.25, mouse_button_mini_pad_width + 11, height],
               rounding=3,
               edges=RIGHT + FRONT,
               anchor=BACK + BOTTOM + LEFT)
            position(BOTTOM)
                cuboid([mouse_button_switch_pad_bar_width,
                        mouse_button_mini_pad_width + 10,
                        mouse_button_switch_pad_bar_height],
                       anchor=TOP);;

    translate([8.8, -7.6 + mouse_button_main_pad_width / 2, mouse_button_switch_pad_bar_height])
        // left button pad
        cuboid([7, mouse_button_main_pad_width + 21.75, 50],
                rounding=3,
                edges=[LEFT + FRONT, LEFT + BACK],
                anchor=LEFT + BACK + BOTTOM) {
            position(LEFT + BACK + BOTTOM)
            ...
            position(RIGHT + FRONT)
                ...
                fwd(2)
                cuboid([47.5, mouse_button_main_pad_width + 11.5, 50],
                    chamfer=3,
                    edges=RIGHT + BOTTOM,
                    anchor=LEFT + FRONT);
        }
    translate([49 + mouse_button_main_pad_width / 2, -15 - mouse_button_main_pad_width / 2, mouse_button_switch_pad_bar_height + 1])
        // right button pad
        cuboid([mouse_button_main_pad_width + 1.5, mouse_button_main_pad_width + 17, 20],
            ...
            position(LEFT + BACK) {
                cuboid([13, mouse_button_main_pad_width + 13, 20],
                    anchor=RIGHT + BACK);
                cuboid([36.5, 8.5, 20],
                    rounding=3,
...
module mouse_buttons_upper_pcb_holder_in_position() {
    ...
    translate(point3d(mouse_buttons_shell_wall_start))
        rotate([mouse_buttons_pcb_x_rotate, 180, thumb_cluster_wall_angle])
            translate([mouse_buttons_and_wheel_x_offset - mouse_buttons_pcb_x_offset - new_x_offset,
                       mouse_buttons_pcb_y_offset + new_y_offset,
                       mouse_buttons_pcb_z_offset + mouse_buttons_pcb_thickness])
...
module mouse_buttons_upper_pcb_holder() {
    ...
    // the cover for the mouse wheel switch (on the thumb tip side of the mouse wheel)
    ...
    diff()
        translate([-wall_thickness, sidecar_upper_length_bottom, -top_shell_thickness])
            cuboid([rear_wall_width,
                    wall_thickness,
                    case_height - bottom_floor_thickness - top_shell_thickness],
                        ...
                        position(BOTTOM + FRONT + LEFT) {
                            down(bottom_floor_thickness)
                                cuboid([wall_thickness, leg_post_lower_diameter + wall_thickness, case_height - top_shell_thickness],
                                    chamfer=side_wall_outer_chamfer,
                                    edges=LEFT + BOTTOM,
                                        ...
                                        cuboid([battery_compartment_internal_width,
                                            ...
                                            position(TOP + FRONT)
                                                ...
                                                cuboid([battery_compartment_internal_width, battery_compartment_cover_floor_catch_thickness, bottom_floor_thickness - battery_compartment_cover_floor_thickness],
                                                       ...
                                                       anchor=BOTTOM + FRONT);
                                        // carve out a additional space under the battery so that the battery is easier to pry out
                                        cuboid([battery_compartment_internal_width,
                                                wall_thickness + battery_compartment_cover_floor_depth - battery_compartment_cover_floor_catch_depth + EXTRA,
                                                5],
                                               anchor=BOTTOM + BACK);
                                    }
...
// uncomment these lines for a dongle compartment test print
// !intersection() {
//     battery_and_dongle_compartment();
//     translate([-5, 0, 7.5])
//         cuboid([25, 25, 30], anchor=BACK + BOTTOM);
// }
...
//     cuboid(10)
//         position(FRONT)
//             assembly_screw_hole_threaded(orient=FWD, slop=horizontal_assembly_screw_slop);
// }
...
module side_wall() {
    diff() {
        side_wall_undecorated() {
            edge_profile(LEFT + BOTTOM)
                    mask2d_chamfer(x=side_wall_outer_chamfer, y=side_wall_outer_chamfer);
            ...
            intersection() {
                zrot(thumb_cluster_wall_angle)
                    ...
                    cuboid([25, // this value just has to be large enough to reach the side wall
                        ...
                        position(RIGHT + FRONT)
                            left(assembly_screw_receiver_diameter / 2)
                            assembly_screw_hole_threaded(FRONT, tag="inner_remove", slop=horizontal_assembly_screw_slop);
...
module mouse_wheel_inner() {
    cyl(d=mouse_wheel_inner_width, h=mouse_wheel_thickness, $fn=6) {
        position(BOTTOM)
            cyl(d=4.6, h=8.9, anchor=TOP, $fn=48*4);
        position(TOP)
            cyl(d=3.3, h=3.9, chamfer2=0.5, anchor=BOTTOM, $fn=48*4)
                position(TOP)
                    cyl(d=2/sqrt(3) * mouse_wheel_hex_diam, h=3.9, anchor=BOTTOM, $fn=6);
    }
}

module mouse_wheel_outer() {
    intersection() {
        diff()
            cyl(d=mouse_wheel_outer_diameter - 2 * mouse_wheel_rib_size,
                h=mouse_wheel_thickness,
                texture="trunc_ribs",
                tex_size=[mouse_wheel_rib_size, mouse_wheel_rib_size])
                position(CENTER)
                    tag("remove")
                    cyl(d=mouse_wheel_inner_width, h=mouse_wheel_thickness + 2 * EXTRA, $fn=6);
        cyl(d=mouse_wheel_outer_diameter,
            h=mouse_wheel_thickness,
            chamfer=mouse_wheel_rib_size);
    }
}

module mouse_wheel_in_position(type) {
    new_x_offset = 72.75;
    new_y_offset = 38.5;
    translate(point3d(mouse_buttons_shell_wall_start, mouse_buttons_additional_height))
        rotate([mouse_buttons_pcb_x_rotate, 180, thumb_cluster_wall_angle])
        translate([mouse_buttons_and_wheel_x_offset - 0.65 - new_x_offset,
                   -wall_thickness - mouse_button_shell_top_thickness + mouse_wheel_pcb_y + new_y_offset,
                   mouse_buttons_pcb_z_offset + mouse_buttons_pcb_thickness + mouse_wheel_elevation]) {
            yrot(-90)
                if (type == "inner")
                    mouse_wheel_inner();
                else if (type == "outer")
...
if (show_foot)
    translate([-rear_wall_width + leg_post_lower_diameter / 2, sidecar_upper_length_bottom - leg_post_lower_diameter, -case_height - 5])
    foot();
if (show_light_cover)
    color("white")
    connection_light_cover_in_position();
if (show_mouse_wheel_2p_inner)
    render()
    color("brown")
    mouse_wheel_in_position("inner");
if (show_mouse_wheel_2p_outer)
    render()
    color("chocolate")
    mouse_wheel_in_position("outer");
if (show_mouse_wheel_1p)
    color("darkgoldenrod")
    render()
    mouse_wheel_in_position("both");
